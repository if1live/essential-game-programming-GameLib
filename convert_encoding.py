...
def convert_encoding(file_path, from_encoding, to_encoding='utf-8'):
    ...
    if chardet.detect(content)['encoding'] == from_encoding:
        content = content.decode(from_encoding).encode(to_encoding)

        # 변환된 내용을 다시 파일에 씁니다.
        with open(file_path, 'wb') as file:
            file.write(content)
        print(f'{file_path} converted from {from_encoding} to {to_encoding}')
    else:
        print(f'{file_path} is not in {from_encoding}')

# 지정된 디렉토리와 하위 디렉토리에 있는 모든 .cpp와 .h 파일을 찾아 인코딩을 변환합니다.
for filepath in Path(path).rglob('*'):
    if filepath.suffix in ['.cpp', '.h']:
        convert_encoding(filepath, 'SHIFT_JIS', 'utf-8')